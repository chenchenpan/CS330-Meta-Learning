...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.meta_train_iterations):
        ...
        if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0:
            ...
            experiments.append({'meta-validation pre-inner-loop accuracy': result[-2], 
            					'meta-validation post-inner-loop accuracy': result[-1],
            					'iter': itr})
            results_df = pd.DataFrame(data=experiments, columns=['meta-validation pre-inner-loop accuracy', 'meta-validation post-inner-loop accuracy', 'iter'])
