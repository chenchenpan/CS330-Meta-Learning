...
if __name__ == '__main__':
	...
	experiments = []
	exp_filename = 'result_proto_meta_val_acc' + '_n_way' + str(n_way) + '_k_shot' + str(k_shot) + '_n_query' + str(n_query) + '_n_meta_test_way' + str(n_meta_test_way) + '_k_meta_test_shot' + str(k_meta_test_shot) + '_n_meta_test_query' + str(n_meta_test_query) + '.csv'
	for ep in range(n_epochs):
		for epi in range(n_episodes):
			...
			support = np.reshape(inputa, [n_way, k_shot, im_width, im_height, channels])
			query = np.reshape(inputb, [n_way, n_query, im_width, im_height, channels])
			...
			if (epi+1) % 50 == 0:
				...
				print('[epoch {}/{}, episode {}/{}] => meta-training loss: {:.5f}, meta-training acc: {:.5f}, meta-val loss: {:.5f}, meta-val acc: {:.5f}'.format(ep+1,
																																		...
																																		val_ls,
																																		val_ac))
				experiments.append({'meta-val accuracy': val_ac,
            						'iter': (epi+1)*(ep+1)})
				# print(experiments)
				results_df = pd.DataFrame(data=experiments, columns=['meta-val accuracy', 'iter'])
				# print(results_df)
	...
	for epi in range(n_meta_test_episodes):
		...
		inputa = image_batches[:, :, :k_meta_test_shot, :]
		inputb = image_batches[:, :, k_meta_test_shot:, :]
		# labela = label_batches[:, :, :k_meta_test_shot, :]
		labelb = label_batches[:, :, k_meta_test_shot:, :]
		
		support = np.reshape(inputa, [n_meta_test_way, k_meta_test_shot, im_width, im_height, channels])
		query = np.reshape(inputb, [n_meta_test_way, n_meta_test_query, im_width, im_height, channels])
		labels = np.reshape(labelb, [n_meta_test_way, n_meta_test_query, n_meta_test_way])
		#############################
