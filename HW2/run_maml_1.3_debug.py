...
## Training options
flags.DEFINE_integer('meta_train_iterations', 150, 'number of meta-training iterations.')
...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    num_samples_per_class = int(data_generator.num_samples_per_class / 2)

    experiments = []
    exp_filename = 'result_' + exp_string + 'maml_meta_val_acc.csv'
    ...
    for itr in range(resume_itr, FLAGS.meta_train_iterations):
        ...
        image_batches, label_batches = data_generator.sample_batch("meta_train", FLAGS.meta_batch_size)
        inputa = image_batches[:, :, :num_samples_per_class, :]
        inputb = image_batches[:, :, num_samples_per_class:, :]
        labela = label_batches[:, :, :num_samples_per_class, :]
        labelb = label_batches[:, :, num_samples_per_class:, :]
        ...
        if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0:
            ...
            # group a (inputa, labela) and group b (inputb, labelb)
            image_batches, label_batches = data_generator.sample_batch("meta_val", FLAGS.meta_batch_size)
            inputa = image_batches[:, :, :num_samples_per_class, :]
            inputb = image_batches[:, :, num_samples_per_class:, :]
            labela = label_batches[:, :, :num_samples_per_class, :]
            labelb = label_batches[:, :, num_samples_per_class:, :]
            ...
            experiments.append({'meta_val_pre-inner_acc': result[-2], 
            					'meta_val_post-inner_acc': result[-1],
            					'iters': itr})
            results_df = pd.DataFrame(data=experiments, columns=['meta_val_pre-inner_acc', 'meta_val_post-inner_acc', 'iters'])
            results_df.to_csv(exp_filename, index=None)
...
def meta_test(model, saver, sess, exp_string, data_generator, meta_test_num_inner_updates=None):
    ...
    for _ in range(NUM_META_TEST_POINTS):
        ...
        # group a (inputa, labela) and group b (inputb, labelb)
        image_batches, label_batches = data_generator.sample_batch("meta_test", FLAGS.meta_batch_size)
        inputa = image_batches[:, :, :num_samples_per_class, :]
        inputb = image_batches[:, :, num_samples_per_class:, :]
        labela = label_batches[:, :, :num_samples_per_class, :]
        labelb = label_batches[:, :, num_samples_per_class:, :]
...
def main():
    ...
    # images, labels = data_generator.sample_batch('meta_train', FLAGS.meta_batch_size)
    # print('*' * 20)
    ...
    # print(model.construct_model().weights)
    ...
    exp_string = 'debug_cls_'+str(FLAGS.n_way)+'.mbs_'+str(FLAGS.meta_batch_size) + '.k_shot_' + str(FLAGS.meta_train_k_shot) + '.inner_numstep' + str(FLAGS.num_inner_updates) + '.inner_updatelr' + str(FLAGS.meta_train_inner_update_lr)
